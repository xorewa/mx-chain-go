...
func runDRWAOrdinaryProcessorVector(
	...
	classificationAccount := loadDRWAJournalAccount(t, accountsDB, vmcommon.SystemAccountAddress)
	require.NoError(t, classificationAccount.AccountDataHandler().SaveKeyValue(
		[]byte("ordinary-control-populated-trie"), []byte{0x01},
	))
	require.NoError(t, accountsDB.SaveAccount(classificationAccount))
