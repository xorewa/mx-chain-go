...
const drwaOrdinaryControlFunction = "prototypeOrdinaryControl"

func TestDRWAAccountingSeamPreservesOrdinaryBuiltInAcrossBothProcessors(t *testing.T) {
	processors := []struct {
		name   string
	...
	for _, processorCase := range processors {
		t.Run(processorCase.name, func(t *testing.T) {
			processor, accountsDB, tokenID, destinationAddress, forwarded, observations := newDRWADestinationProcessorFixture(
				t, processorCase.create, true, false, false, false,
			)
			ordinarySender := bytes.Repeat([]byte{0x11}, drwaAddressLength)
			scr := &smartContractResult.SmartContractResult{
				...
				GasPrice:       1,
				GasLimit:       100,
				Data:           []byte(drwaOrdinaryControlFunction),
				OriginalTxHash: bytes.Repeat([]byte{0x61}, drwaHashLength),
				PrevTxHash:     bytes.Repeat([]byte{0x61}, drwaHashLength),
			...
			canonical := loadDRWAJournalAccount(t, accountsDB, destinationAddress)
...
func TestDRWADestinationProcessorSuccessAndRollbackControls(t *testing.T) {
	processors := []struct {
		name   string
		create func(args scrCommon.ArgsNewSmartContractProcessor) (drwaResultProcessor, error)
	}{
	...
	tests := []struct {
		...
		{name: "no injection success", admitted: true, wantBalance: 2, wantFunction: DRWASettlementReceiptFunction, wantReturnCode: vmcommon.Ok, wantForwardGas: 70, wantFeeCalls: 1, wantRefunded: []uint64{0}},
		{name: "receiver denial", admitted: false, wantBalance: 0, wantFunction: DRWARefundEnvelopeFunction, wantReturnCode: vmcommon.Ok, wantRevert: true, wantForwardGas: 60, wantFeeCalls: 1, wantRefunded: []uint64{0}},
		{name: "after credit output failure", admitted: true, postCreditFault: true, wantBalance: 0, wantFunction: DRWARefundEnvelopeFunction, wantReturnCode: vmcommon.Ok, wantRevert: true, wantForwardGas: 60, wantFeeCalls: 1, wantRefunded: []uint64{0}},
		{name: "wrapped baseline frozen account failure", admitted: true, initialFrozen: true, wantBalance: 0, wantFunction: DRWARefundEnvelopeFunction, wantReturnCode: vmcommon.Ok, wantRevert: true, wantForwardGas: 60, wantFeeCalls: 1, wantRefunded: []uint64{0}, wantReturnText: vmcommonBuiltInFunctions.ErrESDTIsFrozenForAccount.Error()},
	...
	for _, processorCase := range processors {
		...
		for _, test := range tests {
			...
			t.Run(processorCase.name+"/"+test.name, func(t *testing.T) {
				...
				scr := newDRWADestinationSCR(t, tokenID, destinationAddress)
...
func newDRWADestinationProcessorFixture(
	t *testing.T,
	create func(args scrCommon.ArgsNewSmartContractProcessor) (drwaResultProcessor, error),
	admitted bool,
	postCreditFault bool,
	accountingFault bool,
	initialFrozen bool,
) (drwaResultProcessor, *drwaTrackingAccountsDB, []byte, []byte, *[]*smartContractResult.SmartContractResult, *drwaProcessorGasObservations) {
	t.Helper()
	...
	require.NoError(t, destinationAccount.AccountDataHandler().SaveKeyValue(
		drwa.ReceiverGateStorageKey(tokenID), receiverBytes,
	))
	if initialFrozen {
	...
	destination, err := newDRWADestination(drwaDestinationArgs{
		...
		retainedWorkBudgetsProvider: func(identity [32]byte) (drwa.WorkBudgets, uint64, error) {
			...
			return drwa.WorkBudgets{DestinationGate: 10, SuccessReceipt: 20, RefundGeneration: 30, SourceCompletion: 40}, 100, nil
		},
	})
	require.NoError(t, err)
	if postCreditFault {
	...
	hook = &testscommon.BlockChainHookStub{
		...
		ProcessBuiltInFunctionCalled: func(input *vmcommon.ContractCallInput) (*vmcommon.VMOutput, error) {
			if input.Function == drwaOrdinaryControlFunction {
	...
	args := newDRWADestinationProcessorArgs(t, accountsDB, coordinator, enableEpochs, hook, destination, &forwarded, observations)
...
func newDRWADestinationSCR(t *testing.T, tokenID, destinationAddress []byte) *smartContractResult.SmartContractResult {
	t.Helper()
	sourceAddress := bytes.Repeat([]byte{0x11}, drwaAddressLength)
	artifacts, err := drwa.BuildDirectValueArtifacts([32]byte{1}, [32]byte{3}, drwa.DirectValueIntent{
...
func newDRWADestinationProcessorArgs(
	t *testing.T,
	...
	require.NoError(t, builtIns.Add(DRWASettlementReceiptFunction, &processMock.BuiltInFunctionStub{}))
	require.NoError(t, builtIns.Add(DRWARefundEnvelopeFunction, &processMock.BuiltInFunctionStub{}))
	require.NoError(t, builtIns.Add(drwaOrdinaryControlFunction, &processMock.BuiltInFunctionStub{}))
...
func bytesToDRWAAddress(address []byte) [32]byte {
	result := [32]byte{}
	copy(result[:], address)
	return result
}
