...
func TestDRWALifecycleSourceDestinationReceiptCompletion(t *testing.T) {
	...
	sourceHandler := newDRWASourceDataHandler(sourceStored, nil)
	sourceAccount := newDRWASourceAccount(sourceAddress, sourceHandler)
	...
	sourceDebit := newDRWASourceDebitForTest(t, sourceDelegate, func(_ []byte) (bool, error) { return true, nil })
	require.NoError(t, sourceDebit.setBlockchainHook(&testscommon.BlockChainHookStub{CurrentRoundCalled: func() uint64 { return 7 }}))
	sourceInput := newDRWASourceInput(sourceAddress, destinationAddress)
	...
	envelopeBytes := decodeDRWACarrierPayload(t, sourceCarrier.Data, vmcommon.BuiltInFunctionDRWARegulatedValueEnvelope)
	...
	destinationStored := map[string][]byte{
		string(drwa.ReceiverGateStorageKey(envelope.Context.RegulatedTokenID)): receiverBytes,
	}
	destinationAccount := newDRWASourceAccount(
		destinationAddress,
		newDRWASourceDataHandler(destinationStored, nil),
	)
	...
	destination, err := newDRWADestination(drwaDestinationArgs{
		...
		cebEpoch:            9,
		retainedWorkBudgetsProvider: func(_ [32]byte) (drwa.WorkBudgets, uint64, error) {
	...
	destinationIdentity := bytes.Repeat([]byte{0x44}, drwaHashLength)
	...
	receiptBytes := decodeDRWACarrierPayload(t, receiptCarrier.Data, DRWASettlementReceiptFunction)
	...
	completionInput := &vmcommon.ContractCallInput{
		...
		VMInput: vmcommon.VMInput{
			...
			CurrentTxHash:    bytes.Repeat([]byte{0x55}, drwaHashLength),
			OriginalTxHash:   sourceInput.OriginalTxHash,
...
func TestDRWALifecycleDestinationDenialRefundCompletion(t *testing.T) {
	destination, destinationInput, destinationAccount, artifacts := newDRWADestinationFixture(t, false)
	destinationOutput, err := destination.ProcessBuiltinFunction(nil, destinationAccount, destinationInput)
	require.NoError(t, err)
	...
	refundBytes := decodeDRWACarrierPayload(t, refundCarrier.Data, DRWARefundEnvelopeFunction)

	effectBytes, err := drwa.EncodeOpenEffect(artifacts.OpenEffect)
	require.NoError(t, err)
	stored := map[string][]byte{
		string(drwa.OpenEffectStorageKey(artifacts.OpenEffect.EffectID)): effectBytes,
	}
	sourceHandler := newDRWASourceDataHandler(stored, nil)
	sourceAccount := newDRWASourceAccount(sourceAddress, sourceHandler)
	refundApplied := false
	completion, err := newDRWASourceCompletion(drwaSourceCompletionArgs{
		delegate: &processMock.BuiltInFunctionStub{ProcessBuiltinFunctionCalled: func(
	...
	completionOutput, err := completion.ProcessBuiltinFunction(nil, sourceAccount, &vmcommon.ContractCallInput{
		...
		VMInput: vmcommon.VMInput{
			...
			CurrentTxHash:    bytes.Repeat([]byte{0x66}, drwaHashLength),
