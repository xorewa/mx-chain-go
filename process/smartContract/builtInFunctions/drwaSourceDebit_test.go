...
func TestDRWASourceDebitUsesBaselineESDTDelegateForActualDebit(t *testing.T) {
	t.Parallel()
	...
	sourceDebit := newDRWASourceDebitForTest(t, delegate, func(_ []byte) (bool, error) { return true, nil })
	require.NoError(t, sourceDebit.setBlockchainHook(&testscommon.BlockChainHookStub{CurrentRoundCalled: func() uint64 { return 7 }}))
	output, err := sourceDebit.ProcessBuiltinFunction(sourceAccount, sourceAccount, newDRWASourceInput(sourceAddress, destination))
	require.NoError(t, err)
...
func TestDRWASourceDebitCreatesOpenEffectThenDebitsAndEmitsOneCarrier(t *testing.T) {
	t.Parallel()

	events := make([]string, 0, 2)
	stored := make(map[string][]byte)
	dataHandler := newDRWASourceDataHandler(stored, func() { events = append(events, "open-effect") })
	sourceAddress := bytes.Repeat([]byte{0x11}, drwaAddressLength)
	destination := bytes.Repeat([]byte{0x22}, drwaAddressLength)
	sourceAccount := newDRWASourceAccount(sourceAddress, dataHandler)
	...
	input := newDRWASourceInput(sourceAddress, destination)
	...
	effect, err := drwa.LoadOpenEffect(dataHandler, envelope.Context.EffectID)
	require.NoError(t, err)
	...
	require.NoError(t, drwa.ValidateDirectValueOpenEffectContext([32]byte{1}, *effect, envelope.Context))
}

func TestDRWASourceDebitDeniesAdmissionAndAvailabilityBeforeMutation(t *testing.T) {
	t.Parallel()

	errBudgetUnavailable := errors.New("budget unavailable")
	tests := []struct {
		name   string
		mutate func(sourceDebit *drwaSourceDebit, input *vmcommon.ContractCallInput)
	}{
		{name: "activation disabled", mutate: func(sourceDebit *drwaSourceDebit, _ *vmcommon.ContractCallInput) {
			sourceDebit.enableEpochsHandler = enableEpochsHandlerMock.NewEnableEpochsHandlerStub()
		}},
		{name: "unknown origin", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.NativeCallOrigin = vmcommon.NativeCallOriginUnknown
		}},
		{name: "callback", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.CallType = vmData.AsynchronousCallBack
		}},
		{name: "nil call value", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.CallValue = nil }},
		{name: "nonzero EGLD", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.CallValue = big.NewInt(1) }},
		{name: "gas locked", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.GasLocked = 1 }},
		{name: "wrong function", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.Function = "ESDTTransfer" }},
		{name: "caller not source", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.CallerAddr[0] ^= 0xff }},
		{name: "recipient not source", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.RecipientAddr[0] ^= 0xff }},
		{name: "return after error", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.ReturnCallAfterError = true }},
		{name: "async arguments", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.AsyncArguments = &vmcommon.AsyncArguments{CallID: []byte{1}}
		}},
		{name: "parsed ESDT transfers", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.ESDTTransfers = []*vmcommon.ESDTTransfer{{}}
		}},
		{name: "unequal hashes", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.PrevTxHash[0] ^= 0xff }},
		{name: "zero hashes", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			zeroHash := make([]byte, drwaHashLength)
		...
		{name: "wrong hash length", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.CurrentTxHash = input.CurrentTxHash[:drwaHashLength-1]
		}},
		{name: "wrong argument count", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.Arguments = input.Arguments[:2]
		}},
		{name: "same shard destination", mutate: func(sourceDebit *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			sourceDebit.shardCoordinator = testscommon.NewMultiShardsCoordinatorMock(1)
		}},
		{name: "smart contract destination", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.Arguments[0] = make([]byte, drwaAddressLength)
		}},
		{name: "invalid token", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) {
			input.Arguments[1] = []byte("invalid")
		}},
		{name: "nonminimal quantity", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.Arguments[2] = []byte{0, 1} }},
		{name: "zero network domain", mutate: func(sourceDebit *drwaSourceDebit, _ *vmcommon.ContractCallInput) {
			sourceDebit.networkDomain = [32]byte{}
		}},
		{name: "zero CEB", mutate: func(sourceDebit *drwaSourceDebit, _ *vmcommon.ContractCallInput) { sourceDebit.cebEpoch = 0 }},
		{name: "zero lifetime", mutate: func(sourceDebit *drwaSourceDebit, _ *vmcommon.ContractCallInput) {
			sourceDebit.settlementLifetimeRounds = 0
		}},
		{name: "budget unavailable", mutate: func(sourceDebit *drwaSourceDebit, _ *vmcommon.ContractCallInput) {
			sourceDebit.currentWorkBudgetsProvider = func() ([32]byte, drwa.WorkBudgets, uint64, error) {
				return [32]byte{}, drwa.WorkBudgets{}, 0, errBudgetUnavailable
			}
		}},
		{name: "gas equals reserve", mutate: func(_ *drwaSourceDebit, input *vmcommon.ContractCallInput) { input.GasProvided = 100 }},
		{name: "round unavailable", mutate: func(sourceDebit *drwaSourceDebit, _ *vmcommon.ContractCallInput) {
			sourceDebit.blockchainHook = nil
		}},
		{name: "expiry overflow", mutate: func(sourceDebit *drwaSourceDebit, _ *vmcommon.ContractCallInput) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			stored := make(map[string][]byte)
			writeCount := 0
			dataHandler := newDRWASourceDataHandler(stored, func() { writeCount++ })
			sourceAddress := bytes.Repeat([]byte{0x11}, drwaAddressLength)
			destination := bytes.Repeat([]byte{0x22}, drwaAddressLength)
			sourceAccount := newDRWASourceAccount(sourceAddress, dataHandler)
			delegateCalled := false
			...
			require.ErrorIs(t, err, ErrDRWASourceDebitDenied)
...
func TestDRWASourceDebitDeniesUnregulatedAndClassificationFailureBeforeMutation(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			sourceDebit := newDRWASourceDebitForTest(t, delegate, test.classifier)
			require.NoError(t, sourceDebit.setBlockchainHook(&testscommon.BlockChainHookStub{}))

			_, err := sourceDebit.ProcessBuiltinFunction(sourceAccount, sourceAccount, newDRWASourceInput(sourceAddress, destination))
			require.ErrorIs(t, err, ErrDRWASourceDebitDenied)
...
func TestDRWASourceDebitDeniesInvalidAccountRoutesBeforeMutation(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name   string
		invoke func(sourceDebit *drwaSourceDebit, dataHandler vmcommon.AccountDataHandler) (*vmcommon.VMOutput, error)
	}{
		{name: "nil input", invoke: func(sourceDebit *drwaSourceDebit, dataHandler vmcommon.AccountDataHandler) (*vmcommon.VMOutput, error) {
		...
		{name: "nil source account", invoke: func(sourceDebit *drwaSourceDebit, _ vmcommon.AccountDataHandler) (*vmcommon.VMOutput, error) {
			source := bytes.Repeat([]byte{0x11}, drwaAddressLength)
			return sourceDebit.ProcessBuiltinFunction(nil, newDRWASourceAccount(source, nil), newDRWASourceInput(source, bytes.Repeat([]byte{0x22}, drwaAddressLength)))
		}},
		{name: "nil destination account", invoke: func(sourceDebit *drwaSourceDebit, dataHandler vmcommon.AccountDataHandler) (*vmcommon.VMOutput, error) {
			source := bytes.Repeat([]byte{0x11}, drwaAddressLength)
			return sourceDebit.ProcessBuiltinFunction(newDRWASourceAccount(source, dataHandler), nil, newDRWASourceInput(source, bytes.Repeat([]byte{0x22}, drwaAddressLength)))
		}},
		{name: "destination account not source", invoke: func(sourceDebit *drwaSourceDebit, dataHandler vmcommon.AccountDataHandler) (*vmcommon.VMOutput, error) {
			source := bytes.Repeat([]byte{0x11}, drwaAddressLength)
			other := bytes.Repeat([]byte{0x44}, drwaAddressLength)
			return sourceDebit.ProcessBuiltinFunction(newDRWASourceAccount(source, dataHandler), newDRWASourceAccount(other, nil), newDRWASourceInput(source, bytes.Repeat([]byte{0x22}, drwaAddressLength)))
		}},
		{name: "wrong source address length", invoke: func(sourceDebit *drwaSourceDebit, dataHandler vmcommon.AccountDataHandler) (*vmcommon.VMOutput, error) {
			source := bytes.Repeat([]byte{0x11}, drwaAddressLength-1)
			account := newDRWASourceAccount(source, dataHandler)
			return sourceDebit.ProcessBuiltinFunction(account, account, newDRWASourceInput(source, bytes.Repeat([]byte{0x22}, drwaAddressLength)))
		}},
		{name: "smart contract source", invoke: func(sourceDebit *drwaSourceDebit, dataHandler vmcommon.AccountDataHandler) (*vmcommon.VMOutput, error) {
			source := make([]byte, drwaAddressLength)
			account := newDRWASourceAccount(source, dataHandler)
			return sourceDebit.ProcessBuiltinFunction(account, account, newDRWASourceInput(source, bytes.Repeat([]byte{0x22}, drwaAddressLength)))
		}},
	}
...
func TestDRWASourceDebitReturnsMutationErrorForStorageDelegateAndPostDebitFailures(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:        "baseline debit",
			dataHandler: newDRWASourceDataHandler(make(map[string][]byte), nil),
			delegate: &processMock.BuiltInFunctionStub{ProcessBuiltinFunctionCalled: func(_, _ vmcommon.UserAccountHandler, _ *vmcommon.ContractCallInput) (*vmcommon.VMOutput, error) {
		...
		},
		{
			...
			wantCause: ErrInvalidDRWASourceDebitDelegate,
		},
	}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			sourceAddress := bytes.Repeat([]byte{0x11}, drwaAddressLength)
			destination := bytes.Repeat([]byte{0x22}, drwaAddressLength)
			sourceAccount := newDRWASourceAccount(sourceAddress, test.dataHandler)
			sourceDebit := newDRWASourceDebitForTest(t, test.delegate, func(_ []byte) (bool, error) { return true, nil })
			require.NoError(t, sourceDebit.setBlockchainHook(&testscommon.BlockChainHookStub{}))

			output, err := sourceDebit.ProcessBuiltinFunction(sourceAccount, sourceAccount, newDRWASourceInput(sourceAddress, destination))
			require.Nil(t, output)
			require.ErrorIs(t, err, ErrDRWASourceDebitMutation)
			require.ErrorIs(t, err, test.wantCause)
		})
	}
}

func TestDRWASourceDebitLifecycleAndConstruction(t *testing.T) {
	t.Parallel()

	_, err := newDRWASourceDebit(drwaSourceDebitArgs{})
	require.ErrorIs(t, err, ErrInvalidDRWASourceDebitDelegate)
	...
	require.ErrorIs(t, sourceDebit.setBlockchainHook(nil), ErrInvalidDRWASourceDebitDelegate)
	require.False(t, sourceDebit.IsInterfaceNil())
	var nilSourceDebit *drwaSourceDebit
	require.True(t, nilSourceDebit.IsInterfaceNil())
}

func newDRWASourceDebitForTest(
	t *testing.T,
	delegate vmcommon.BuiltinFunction,
	classifier drwaTokenClassifier,
) *drwaSourceDebit {
...
func newDRWASourceInput(source, destination []byte) *vmcommon.ContractCallInput {
...
func newDRWASourceAccount(address []byte, dataHandler vmcommon.AccountDataHandler) *vmcommonMocks.UserAccountStub {
...
func newDRWASourceDataHandler(stored map[string][]byte, onSave func()) *trieMock.DataTrieTrackerStub {
