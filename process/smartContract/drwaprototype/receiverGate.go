...
const (
	prototypeReceiverGateVersion   = byte(1)
	prototypeReceiverGateKeySuffix = "drwa/receiver-gate/v1/"
)
...
type ReceiverGateRecord struct {
	Holder            [prototypeAddressLength]byte
	CEBEpoch          uint32
	Admitted          bool
...
func ReceiverGateStorageKey(tokenID []byte) []byte {
	key := make([]byte, 0, len(core.ProtectedKeyPrefix)+len(prototypeReceiverGateKeySuffix)+len(tokenID))
	key = append(key, core.ProtectedKeyPrefix...)
	key = append(key, prototypeReceiverGateKeySuffix...)
...
func EncodeReceiverGateRecord(record ReceiverGateRecord) ([]byte, error) {
	if record.Holder == ([prototypeAddressLength]byte{}) || record.CEBEpoch == 0 || record.ValidThroughRound == 0 {
		return nil, ErrInvalidReceiverGate
	}

	encoded := make([]byte, 0, 1+prototypeAddressLength+4+1+8)
	encoded = append(encoded, prototypeReceiverGateVersion)
...
func DecodeReceiverGateRecord(encoded []byte) (*ReceiverGateRecord, error) {
	const encodedLength = 1 + prototypeAddressLength + 4 + 1 + 8
	if len(encoded) != encodedLength || encoded[0] != prototypeReceiverGateVersion {
		return nil, ErrInvalidReceiverGate
	}
	record := &ReceiverGateRecord{}
	copy(record.Holder[:], encoded[1:1+prototypeAddressLength])
	record.CEBEpoch = binary.BigEndian.Uint32(encoded[1+prototypeAddressLength : 1+prototypeAddressLength+4])
	admitted := encoded[1+prototypeAddressLength+4]
	if admitted > 1 {
		return nil, ErrInvalidReceiverGate
	}
	record.Admitted = admitted == 1
	record.ValidThroughRound = binary.BigEndian.Uint64(encoded[1+prototypeAddressLength+5:])
	if record.Holder == ([prototypeAddressLength]byte{}) || record.CEBEpoch == 0 || record.ValidThroughRound == 0 {
		return nil, ErrInvalidReceiverGate
	}
...
func LoadReceiverGateRecord(dataHandler vmcommon.AccountDataHandler, tokenID []byte) (*ReceiverGateRecord, error) {
	...
	if !vmcommon.ValidateToken(tokenID) || len(tokenID) > prototypeTokenIDLimit {
		return nil, ErrInvalidReceiverGate
	}
