...
func TestValidateProtocolMessageAdmission(t *testing.T) {
	...
	validCallData := createDRWAEnvelopeCallData(t)
	...
	t.Run("oversized envelope rejects before decode", func(t *testing.T) {
		...
		scr.Data = append(
			...
			bytes.Repeat([]byte{'0'}, 2*drwa.DRWAValueEnvelopeMaximumLength()+2)...,
		)
		require.ErrorIs(t, ValidateProtocolMessageAdmission(scr, enabled, coordinator), process.ErrInvalidProtocolMessageEnvelope)
	})
	...
	t.Run("exact settlement and refund carriers reach source validation", func(t *testing.T) {
		for _, refund := range []bool{false, true} {
			scr := valid()
			scr.Data = createDRWAResultCallData(t, refund)
	...
	t.Run("malformed settlement carrier rejects before account loading", func(t *testing.T) {
		scr := valid()
		scr.Data = []byte(DRWASettlementReceiptFunction + "@00")
		require.ErrorIs(t, ValidateProtocolMessageAdmission(scr, enabled, coordinator), process.ErrInvalidProtocolMessageEnvelope)
	})
}

func createDRWAResultCallData(t *testing.T, refund bool) []byte {
	t.Helper()
	effectID := [32]byte{1}
	contextHash := [32]byte{2}
	destinationIdentity := [32]byte{3}
	function := DRWASettlementReceiptFunction
	receipt, err := drwa.BuildSettlementReceipt([32]byte{4}, effectID, contextHash, destinationIdentity)
...
func createDRWAEnvelopeCallData(t *testing.T) []byte {
	t.Helper()
	...
	encoded, err := drwa.EncodeValueEnvelope(drwa.ValueEnvelope{
		OriginalTransferPayload: []byte("ESDTTransfer@544f4b454e2d616263646566@01"),
		Context:                 context,
	})
	require.NoError(t, err)
