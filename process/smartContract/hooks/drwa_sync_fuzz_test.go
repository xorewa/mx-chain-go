...
func FuzzValidateDRWASyncVersion(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, currentVersion uint64, nextVersion uint64) {
		...
		switch {
		case currentVersion == math.MaxUint64:
			if err == nil || err.Error() != drwaSyncRejectVersionOverflow {
				t.Fatalf("expected overflow rejection for current=%d next=%d, got %v", currentVersion, nextVersion, err)
			}
		...
		case nextVersion == currentVersion:
			if err == nil || err.Error() != drwaSyncRejectReplayDuplicate {
				t.Fatalf("expected duplicate rejection for current=%d next=%d, got %v", currentVersion, nextVersion, err)
			}
		case nextVersion != currentVersion+1:
			if err == nil || err.Error() != drwaSyncRejectVersionGap {
				t.Fatalf("expected version-gap rejection for current=%d next=%d, got %v", currentVersion, nextVersion, err)
			}
		default:
			if err != nil {
