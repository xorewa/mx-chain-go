...
func TestScProcessor_PrototypeSourceDebitFailureShapeRollbackMatrix(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name            string
		failBeforeWrite bool
		writeOpenEffect bool
		debit           bool
	}{
		{name: "before OpenEffect storage", failBeforeWrite: true},
		{name: "during OpenEffect storage", writeOpenEffect: true},
		{name: "at baseline debit", writeOpenEffect: true, debit: true},
		{name: "after debit", writeOpenEffect: true, debit: true},
	}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			const functionName = "DRWAPrototypeTransfer"
			sourceAddress := bytes.Repeat([]byte{0x11}, 32)
			tokenBalance := int64(100)
			openEffectPresent := false
			revertCalled := false
			drwaOutputsForwarded := 0
			builtInCalled := false
			injected := fmt.Errorf("injected %s", test.name)
			...
			tx := &transaction.Transaction{
				...
				Value:    big.NewInt(0),
				GasLimit: 100000,
				Data:     []byte(functionName + "@01@02@03"),
			}
			acntSrc, _ := createAccounts(tx)
			vmSource := acntSrc.(vmcommon.UserAccountHandler)
			accountsDB := &stateMock.AccountsStub{
				JournalLenCalled:  func() int { return 17 },
				LoadAccountCalled: func(_ []byte) (vmcommon.AccountHandler, error) { return acntSrc, nil },
				RevertToSnapshotCalled: func(snapshot int) error {
					require.Equal(t, 17, snapshot)
					revertCalled = true
					tokenBalance = 100
					openEffectPresent = false
					return nil
			...
			builtInFunc := &mock.BuiltInFunctionStub{ProcessBuiltinFunctionCalled: func(_, _ vmcommon.UserAccountHandler, _ *vmcommon.ContractCallInput) (*vmcommon.VMOutput, error) {
				builtInCalled = true
				if test.failBeforeWrite {
					return nil, injected
				}
				openEffectPresent = test.writeOpenEffect
				if test.debit {
					tokenBalance -= 10
				}
				return nil, injected
			...
			arguments.EnableEpochsHandler = enableEpochsHandlerMock.NewEnableEpochsHandlerStub(common.SCDeployFlag, common.BuiltInFunctionsFlag)
			require.NoError(t, arguments.BuiltInFunctions.Add(functionName, builtInFunc))
			arguments.BlockChainHook = &testscommon.BlockChainHookStub{ProcessBuiltInFunctionCalled: func(input *vmcommon.ContractCallInput) (*vmcommon.VMOutput, error) {
				return builtInFunc.ProcessBuiltinFunction(vmSource, vmSource, input)
			}}
			arguments.ScrForwarder = &mock.IntermediateTransactionHandlerMock{AddIntermediateTransactionsCalled: func(txs []data.TransactionHandler, _ []byte) error {
				for _, txResult := range txs {
					scr, ok := txResult.(*smartContractResult.SmartContractResult)
					if ok && scr.GetProtocolMessageKind() == vmData.ProtocolMessageKindDRWA {
						drwaOutputsForwarded++
					}
				}
				return nil
			}}
			...
			_, _ = sc.ExecuteBuiltInFunction(tx, acntSrc, acntSrc)
			require.True(t, builtInCalled)
			require.Equal(t, int64(100), tokenBalance)
			require.False(t, openEffectPresent)
			require.Zero(t, drwaOutputsForwarded)
