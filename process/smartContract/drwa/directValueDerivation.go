...
const (
	drwaNormalizedDirectVersion = byte(1)
	drwaDirectOriginEffectIndex = uint32(0)

	drwaNormalizedDirectDomain = "DRWA/PROTOTYPE/NORMALIZED_DIRECT/v1"
	drwaDirectEffectDomain     = "DRWA/EFFECT/DIRECT/v1"
	drwaValueContextDomain     = "DRWA/VALUE_CONTEXT/v1"
)
...
type DirectValueArtifacts struct {
	NormalizedEffectDigest [drwaDigestLength]byte
...
func BuildDirectValueArtifacts(
	...
	originTxHash [drwaDigestLength]byte,
	...
	transferPayload := buildDRWAESDTTransferPayload(intent.RegulatedTokenID, intent.Quantity)
	normalizedBytes := encodeDRWANormalizedDirectIntent(intent, transferPayload)
	normalizedDigest := sha256.Sum256(appendDomain(drwaNormalizedDirectDomain, normalizedBytes))

	effectPreimage := make([]byte, 0, len(drwaDirectEffectDomain)+3*drwaDigestLength+8)
	effectPreimage = append(effectPreimage, drwaDirectEffectDomain...)
	effectPreimage = append(effectPreimage, networkDomain[:]...)
	effectPreimage = append(effectPreimage, originTxHash[:]...)
	effectPreimage = binary.BigEndian.AppendUint32(effectPreimage, drwaDirectOriginEffectIndex)
	...
	contextPreimage := make([]byte, 0, len(drwaValueContextDomain)+drwaDigestLength+len(contextBytes))
	contextPreimage = append(contextPreimage, drwaValueContextDomain...)
...
func validateDirectValueIntent(
	...
	if isZeroDRWADigest(originTxHash) {
		return fmt.Errorf("%w: zero origin transaction hash", ErrInvalidDirectValueIntent)
	}
	if !vmcommon.ValidateToken(intent.RegulatedTokenID) || len(intent.RegulatedTokenID) > drwaTokenIDLimit {
		return fmt.Errorf("%w: regulated token identifier", ErrInvalidDirectValueIntent)
	}
	if len(intent.Quantity) == 0 || len(intent.Quantity) > drwaQuantityLimit || intent.Quantity[0] == 0 {
	...
	if isZeroDRWADigest(intent.GasScheduleIdentity) {
...
func encodeDRWANormalizedDirectIntent(intent DirectValueIntent, transferPayload []byte) []byte {
	encoded := make([]byte, 0, drwaContextMaximumLength()+len(transferPayload))
	encoded = append(encoded, drwaNormalizedDirectVersion)
...
func buildDRWAESDTTransferPayload(tokenID []byte, quantity []byte) []byte {
...
func DecodeDirectValueTransferPayload(payload []byte) ([]byte, []byte, error) {
	...
	tokenID, err := decodeDRWACanonicalHex(parts[1])
	if err != nil || !vmcommon.ValidateToken(tokenID) || len(tokenID) > drwaTokenIDLimit {
		return nil, nil, ErrInvalidDirectValueIntent
	}
	quantity, err := decodeDRWACanonicalHex(parts[2])
	if err != nil || len(quantity) == 0 || len(quantity) > drwaQuantityLimit || quantity[0] == 0 {
		return nil, nil, ErrInvalidDirectValueIntent
	}
	if !bytes.Equal(payload, buildDRWAESDTTransferPayload(tokenID, quantity)) {
		return nil, nil, ErrInvalidDirectValueIntent
	}
	return tokenID, quantity, nil
}

func decodeDRWACanonicalHex(encoded []byte) ([]byte, error) {
...
func isZeroDRWADigest(value [drwaDigestLength]byte) bool {
	return value == [drwaDigestLength]byte{}
}
