...
func TestValueEnvelopeDRWADeterministicFixture(t *testing.T) {
	t.Parallel()
...
func TestValueEnvelopeDRWADoesNotAliasCallerOrEncodedSlices(t *testing.T) {
	t.Parallel()
...
func TestEncodeValueEnvelopeDRWARejectsUnsupportedOrUnboundedFields(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			mutate: func(envelope *ValueEnvelope) {
				envelope.OriginalTransferPayload = make([]byte, drwaPayloadLimit+1)
			},
		},
		{
		...
		},
		{
			...
			mutate: func(envelope *ValueEnvelope) {
				envelope.Context.RegulatedTokenID = make([]byte, drwaTokenIDLimit+1)
			},
		},
		{
		...
		},
		{
			...
			mutate: func(envelope *ValueEnvelope) {
				envelope.Context.Quantity = make([]byte, drwaQuantityLimit+1)
...
func TestDecodeValueEnvelopeDRWARejectsMalformedBytes(t *testing.T) {
	t.Parallel()
...
func TestDecodeValueEnvelopeDRWARejectsEveryTruncatedPrefix(t *testing.T) {
	t.Parallel()
...
func TestDecodeValueEnvelopeDRWARejectsInvalidDecodedFields(t *testing.T) {
	t.Parallel()
...
func valueEnvelopeFixtureOffsets(fixture ValueEnvelope) valueEnvelopeOffsets {
	...
	effectKind := contextStart + drwaDigestLength
	tokenType := effectKind + 1 + drwaDigestLength + 2 + len(fixture.Context.RegulatedTokenID)
	quantity := tokenType + 1 + 2
	transferMode := quantity + len(fixture.Context.Quantity) + 2*drwaAddressLength + 4
	settlementExpiry := transferMode + 1
	destinationGateGasLimit := settlementExpiry + 8 + drwaDigestLength
