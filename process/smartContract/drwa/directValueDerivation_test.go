...
func TestBuildDirectValueArtifactsDRWADeterministicFixture(t *testing.T) {
	t.Parallel()
	...
	normalizedBytes := encodeDRWANormalizedDirectIntent(intent, artifacts.Envelope.OriginalTransferPayload)
...
func TestValidateDirectValueOpenEffectContextDRWARejectsEveryBindingMismatch(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		name   string
		mutate func(effect *OpenEffect, context *ValueContext, domain *[drwaDigestLength]byte)
	}{
		{name: "network domain", mutate: func(_ *OpenEffect, _ *ValueContext, domain *[drwaDigestLength]byte) { domain[0] ^= 0xff }},
		{name: "effect ID", mutate: func(effect *OpenEffect, _ *ValueContext, _ *[drwaDigestLength]byte) { effect.EffectID[0] ^= 0xff }},
		{name: "origin", mutate: func(effect *OpenEffect, _ *ValueContext, _ *[drwaDigestLength]byte) {
			effect.OriginExecutionIdentity[0] ^= 0xff
		}},
		{name: "source", mutate: func(effect *OpenEffect, _ *ValueContext, _ *[drwaDigestLength]byte) {
			effect.SourceSubject[0] ^= 0xff
		}},
		{name: "CEB epoch", mutate: func(effect *OpenEffect, _ *ValueContext, _ *[drwaDigestLength]byte) { effect.CEBEpoch++ }},
		{name: "gas identity", mutate: func(effect *OpenEffect, _ *ValueContext, _ *[drwaDigestLength]byte) {
			effect.GasScheduleIdentity[0] ^= 0xff
		}},
		{name: "context gas identity", mutate: func(_ *OpenEffect, context *ValueContext, _ *[drwaDigestLength]byte) {
			context.GasScheduleIdentity[0] ^= 0xff
		}},
		{name: "context hash", mutate: func(effect *OpenEffect, _ *ValueContext, _ *[drwaDigestLength]byte) {
...
func TestBuildDirectValueArtifactsDRWAArchitectureFormulaRecomputation(t *testing.T) {
	t.Parallel()
...
func TestBuildDirectValueArtifactsDRWARejectsInvalidInput(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name   string
		mutate func(networkDomain *[drwaDigestLength]byte, originTxHash *[drwaDigestLength]byte, intent *DirectValueIntent)
	}{
		{
			name: "zero network domain",
			mutate: func(networkDomain *[drwaDigestLength]byte, _ *[drwaDigestLength]byte, _ *DirectValueIntent) {
				clear(networkDomain[:])
			},
		},
		{
			name: "zero origin transaction hash",
			mutate: func(_ *[drwaDigestLength]byte, originTxHash *[drwaDigestLength]byte, _ *DirectValueIntent) {
				clear(originTxHash[:])
			},
		},
		{
			name: "empty token identifier",
			mutate: func(_ *[drwaDigestLength]byte, _ *[drwaDigestLength]byte, intent *DirectValueIntent) {
		...
		},
		{
			...
			mutate: func(_ *[drwaDigestLength]byte, _ *[drwaDigestLength]byte, intent *DirectValueIntent) {
				intent.Quantity = make([]byte, drwaQuantityLimit+1)
				intent.Quantity[0] = 1
			},
		},
		{
...
func TestBuildDirectValueArtifactsDRWANormalizedDigestCommitsEveryIntentField(t *testing.T) {
	t.Parallel()
...
func TestBuildDirectValueArtifactsDRWASeparatesOriginAndNetworkBindings(t *testing.T) {
	t.Parallel()
...
func TestBuildDirectValueArtifactsDRWADoesNotAliasInputOrOutputSlices(t *testing.T) {
	t.Parallel()
...
func createDirectValueDerivationFixture() ([drwaDigestLength]byte, [drwaDigestLength]byte, DirectValueIntent) {
	networkDomain := sequentialDRWADigest(1)
	originTxHash := sequentialDRWADigest(33)
	intent := DirectValueIntent{
		RegulatedTokenID:         []byte("TOKEN-abcdef"),
		Quantity:                 []byte{1, 0},
		SourceHolder:             sequentialDRWADigest(65),
		DestinationHolder:        sequentialDRWADigest(97),
		CEBEpoch:                 7,
		SettlementExpiry:         1234,
		GasScheduleIdentity:      sequentialDRWADigest(129),
...
func sequentialDRWADigest(first byte) [drwaDigestLength]byte {
	result := [drwaDigestLength]byte{}
