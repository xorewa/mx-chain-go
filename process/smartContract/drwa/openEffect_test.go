...
func TestOpenEffectDRWADeterministicFixture(t *testing.T) {
	t.Parallel()
...
func TestOpenEffectDRWADoesNotAliasCallerOrEncodedSlices(t *testing.T) {
	t.Parallel()
...
func TestEncodeOpenEffectDRWARejectsUnsupportedFields(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			mutate: func(effect *OpenEffect) {
				effect.RegulatedTokenID = make([]byte, drwaTokenIDLimit+1)
			},
		},
		{
...
func TestDecodeOpenEffectDRWARejectsMalformedBytes(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			name: "zero gas schedule identity",
			mutate: func(value []byte) []byte {
				clear(value[offsets.gasScheduleIdentity : offsets.gasScheduleIdentity+drwaDigestLength])
				return value
			},
		},
		{
...
func TestDecodeOpenEffectDRWARejectsInputAboveTotalLimit(t *testing.T) {
	t.Parallel()

	_, err := DecodeOpenEffect(make([]byte, drwaOpenEffectMaximumLength()+1))
	require.ErrorIs(t, err, ErrInvalidOpenEffect)
}

func TestDecodeOpenEffectDRWARejectsEveryTruncatedPrefix(t *testing.T) {
	t.Parallel()
...
func TestOpenEffectStorageKeyIsFixedProtectedAndFresh(t *testing.T) {
	...
	expectedPrefix := core.ProtectedKeyPrefix + drwaOpenEffectKeySuffix
	require.Equal(t, append([]byte(expectedPrefix), fixture.EffectID[:]...), key)
	require.Len(t, key, len(expectedPrefix)+drwaDigestLength)
...
func TestCreateAndLoadOpenEffectDRWA(t *testing.T) {
	t.Parallel()
...
func TestLoadOpenEffectDRWARejectsAbsenceMalformedAndKeyMismatch(t *testing.T) {
	t.Parallel()
...
func openEffectFixtureOffsets(fixture OpenEffect) openEffectOffsets {
	effectKind := 1 + drwaDigestLength
	tokenLength := effectKind + 1
	tokenType := tokenLength + 2 + len(fixture.RegulatedTokenID)
	gasScheduleIdentity := tokenType + 1 + drwaDigestLength + drwaAddressLength + 4
	terminalKind := gasScheduleIdentity + 2*drwaDigestLength
