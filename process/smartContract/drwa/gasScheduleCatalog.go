...
const (
	drwaGasScheduleProfileLimit = 256
	drwaGasScheduleMapDomain    = "DRWA/PROTOTYPE/GAS_SCHEDULE/v1"
	drwaGasCatalogDomain        = "DRWA/PROTOTYPE/GAS_CATALOG/v1"

	// PrototypeWorkBudgetSection is the dedicated non-normative S1 gas-schedule section.
	DRWAWorkBudgetSection = "DRWAPrototypeCost"
	// DRWADestinationGateCost is the configured destination-gate work key.
	DRWADestinationGateCost = "DestinationGate"
	// DRWASuccessReceiptCost is the configured success-receipt work key.
	DRWASuccessReceiptCost = "SuccessReceipt"
	// DRWARefundGenerationCost is the configured refund-generation work key.
	DRWARefundGenerationCost = "RefundGeneration"
	// DRWASourceCompletionCost is the configured source-completion work key.
	DRWASourceCompletionCost = "SourceCompletion"
)
...
func SealGasScheduleCatalog(profiles []GasScheduleProfile) (*GasScheduleCatalog, error) {
	if len(profiles) == 0 || len(profiles) > drwaGasScheduleProfileLimit {
	...
	catalog := &GasScheduleCatalog{
		...
		schedules: make(map[[drwaDigestLength]byte]GasScheduleMap),
	}
	canonicalMaps := make(map[[drwaDigestLength]byte][]byte)
...
func GasScheduleIdentity(schedule GasScheduleMap) ([drwaDigestLength]byte, error) {
	identity, _, err := canonicalGasScheduleIdentity(schedule)
	return identity, err
}

// Identity returns the sealed activation-timeline identity.
func (catalog *GasScheduleCatalog) Identity() ([drwaDigestLength]byte, error) {
	if catalog == nil {
		return [drwaDigestLength]byte{}, fmt.Errorf("%w: nil catalog", ErrInvalidGasScheduleCatalog)
	}
...
func (catalog *GasScheduleCatalog) IdentityAtEpoch(epoch uint32) ([drwaDigestLength]byte, error) {
	if catalog == nil || len(catalog.entries) == 0 {
		return [drwaDigestLength]byte{}, fmt.Errorf("%w: unavailable timeline", ErrInvalidGasScheduleCatalog)
	}
...
func (catalog *GasScheduleCatalog) Schedule(identity [drwaDigestLength]byte) (GasScheduleMap, error) {
	if catalog == nil {
...
func (catalog *GasScheduleCatalog) MaximumWorkBudgets() (WorkBudgets, error) {
	...
	for index, entry := range catalog.entries {
		...
		section, exists := schedule[DRWAWorkBudgetSection]
		if !exists {
...
func workBudgetsFromSection(section map[string]uint64) (WorkBudgets, error) {
	...
	budgets := WorkBudgets{
		DestinationGate:  section[DRWADestinationGateCost],
		SuccessReceipt:   section[DRWASuccessReceiptCost],
		RefundGeneration: section[DRWARefundGenerationCost],
		SourceCompletion: section[DRWASourceCompletionCost],
	}
	_, err := budgets.Total()
	if err != nil {
		return WorkBudgets{}, err
...
func canonicalGasScheduleIdentity(schedule GasScheduleMap) ([drwaDigestLength]byte, []byte, error) {
	if len(schedule) == 0 {
		return [drwaDigestLength]byte{}, nil, fmt.Errorf("%w: empty schedule", ErrInvalidGasScheduleCatalog)
	}
	...
	for section, operations := range schedule {
		if len(section) == 0 || len(section) > math.MaxUint16 || len(operations) == 0 {
			return [drwaDigestLength]byte{}, nil, fmt.Errorf("%w: section", ErrInvalidGasScheduleCatalog)
		}
	...
	for _, section := range sections {
		...
		for operation := range operationsMap {
			if len(operation) == 0 || len(operation) > math.MaxUint16 {
				return [drwaDigestLength]byte{}, nil, fmt.Errorf("%w: operation", ErrInvalidGasScheduleCatalog)
			}
	...
	preimage := append([]byte(drwaGasScheduleMapDomain), canonical...)
	return sha256.Sum256(preimage), canonical, nil
}

func deriveGasScheduleCatalogIdentity(entries []sealedGasScheduleEntry) [drwaDigestLength]byte {
	preimage := make([]byte, 0, len(drwaGasCatalogDomain)+4+len(entries)*(4+drwaDigestLength))
	preimage = append(preimage, drwaGasCatalogDomain...)
