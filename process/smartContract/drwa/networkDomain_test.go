...
func TestDeriveNetworkDomainDRWADeterministicFixture(t *testing.T) {
	t.Parallel()

	chainID := []byte("local-testnet")
	genesisHash := sequentialDRWADigest(161)
...
func TestDeriveNetworkDomainDRWARejectsEmptyChainID(t *testing.T) {
	t.Parallel()

	domain, err := DeriveNetworkDomain(nil, sequentialDRWADigest(1))
	require.Equal(t, [drwaDigestLength]byte{}, domain)
	require.ErrorIs(t, err, ErrInvalidNetworkDomainInput)
}

func TestDeriveNetworkDomainDRWACommitsEveryInputByte(t *testing.T) {
	t.Parallel()
...
func TestDeriveNetworkDomainDRWAAcceptsFixedWidthZeroHash(t *testing.T) {
	t.Parallel()

	domain, err := DeriveNetworkDomain([]byte("local-testnet"), [drwaDigestLength]byte{})
	require.NoError(t, err)
	require.NotEqual(t, [drwaDigestLength]byte{}, domain)
}
