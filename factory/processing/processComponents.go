...
func (pcf *processComponentsFactory) generateGenesisHeadersAndApplyInitialBalances() (map[uint32]data.HeaderHandler, map[uint32]*genesis.IndexingData, error) {
	...
	err := validatePrototypeDRWAReceiverSeedingMode(
		pcf.config.BuiltInFunctions.PrototypeDRWAReceiverSeeds,
		pcf.config.Hardfork,
		startEpoch,
	)
	if err != nil {
	...
	arg := processGenesis.ArgsGenesisBlockCreator{
		...
		PrototypeReceiverSeeds: append(
			[]config.PrototypeDRWAReceiverSeedConfig(nil),
			pcf.config.BuiltInFunctions.PrototypeDRWAReceiverSeeds...,
		),
	}
...
func validatePrototypeDRWAReceiverSeedingMode(
	...
	if hardForkConfig.AfterHardFork && startEpoch <= hardForkConfig.StartEpoch {
		return fmt.Errorf("%w: non-empty list cannot be combined with hard-fork import", processGenesis.ErrInvalidPrototypeDRWAReceiverSeeds)
	}
