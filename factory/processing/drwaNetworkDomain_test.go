...
type drwaIdentityCandidateStorer struct {
	*storageTests.StorerStub
	candidates    map[string][]byte
	candidatesErr error
}

func (storer *drwaIdentityCandidateStorer) DRWANetworkIdentityCandidates(key []byte) (map[string][]byte, error) {
	if storer.candidatesErr != nil {
		return nil, storer.candidatesErr
	}
	result := make(map[string][]byte, len(storer.candidates)+1)
	for name, value := range storer.candidates {
		result[name] = append([]byte(nil), value...)
	}
	if storer.StorerStub != nil && storer.GetCalled != nil {
		global, err := storer.Get(key)
		if err == nil {
			result["global"] = append([]byte(nil), global...)
...
func newDRWAIdentityCandidateStore(
	global map[string][]byte,
	candidates map[string][]byte,
	putCalls *int,
	readBackMismatch bool,
) dataRetriever.StorageService {
	storer := &drwaIdentityCandidateStorer{
		StorerStub: &storageTests.StorerStub{
			GetCalled: func(key []byte) ([]byte, error) {
				value, ok := global[string(key)]
				if !ok {
					return nil, storage.ErrKeyNotFound
				}
				return append([]byte(nil), value...), nil
	...
	chainStore.AddStorer(dataRetriever.DRWANetworkIdentityUnit, storer)
	return chainStore
}
...
func TestResolveDRWANetworkDomainRejectsCandidateEnumerationFailure(t *testing.T) {
	expectedErr := errors.New("candidate enumeration failed")
	store := dataRetriever.NewChainStorer()
	store.AddStorer(dataRetriever.DRWANetworkIdentityUnit, &drwaIdentityCandidateStorer{
		StorerStub:    &storageTests.StorerStub{},
		candidatesErr: expectedErr,
	})
...
func TestResolveDRWANetworkDomainRejectsAnyInvalidOrConflictingCandidate(t *testing.T) {
	chainID := "localnet"
	marshalizer := &marshal.GogoProtoMarshalizer{}
	header := drwaMetaGenesisHeader(chainID)
	headerBytes, err := marshalizer.Marshal(header)
	...
	require.NoError(t, err)
	otherHeader := drwaMetaGenesisHeader(chainID)
	otherHeader.TimeStamp++
	otherHeaderBytes, err := marshalizer.Marshal(otherHeader)
	require.NoError(t, err)
	conflicting, err := encodeDRWANetworkIdentity(drwaIdentityForHeaderBytes(
		chainID, 0, drwaNetworkIdentityProvenanceLocalCanonicalGenesis, otherHeaderBytes,
	))
	require.NoError(t, err)

	tests := map[string]map[string][]byte{
		"conflicting valid legacy": {
			"legacy/Shard_0": valid,
			"legacy/Shard_1": conflicting,
		},
		"valid global plus malformed legacy": {
			"global":         valid,
			"legacy/Shard_2": []byte("malformed"),
		},
	}
	for name, candidates := range tests {
		t.Run(name, func(t *testing.T) {
			...
			for candidateName, value := range candidates {
				if candidateName == "global" {
					global[string(drwaNetworkIdentityKey(0))] = value
				} else {
					legacy[candidateName] = value
				}
			}
			store := newDRWAIdentityCandidateStore(global, legacy, nil, false)
			_, _, _, resolveErr := resolveDRWANetworkDomain(
				chainID,
				map[uint32]data.HeaderHandler{core.MetachainShardId: &block.MetaBlock{}},
				0,
				4,
...
func TestResolveDRWANetworkDomainRejectsFreshGenesisReadbackMismatch(t *testing.T) {
	...
	marshalizer := &marshal.GogoProtoMarshalizer{}
	header := drwaMetaGenesisHeader(chainID)
	store := newDRWAIdentityCandidateStore(make(map[string][]byte), nil, nil, true)
