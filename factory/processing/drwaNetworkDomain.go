...
func resolveDRWANetworkDomain(
	...
	storedEnvelope, identity, canonicalHash, networkDomain, found, err := loadAndValidateDRWANetworkIdentity(
		storageService,
		key,
		chainID,
		canonicalEpoch,
	...
	readBack, err := storageService.Get(dataRetriever.DRWANetworkIdentityUnit, key)
	if err != nil {
		return [32]byte{}, [32]byte{}, 0, fmt.Errorf("%w: read back local canonical genesis: %w", errInvalidDRWANetworkIdentity, err)
	}
	if !bytes.Equal(readBack, envelope) {
		return [32]byte{}, [32]byte{}, 0, fmt.Errorf("%w: local canonical genesis readback mismatch", errInvalidDRWANetworkIdentity)
	}
	log.Info(formatDRWANetworkIdentityObservation(
		canonicalEpoch,
		bootstrapEpoch,
...
type drwaNetworkIdentityCandidateProvider interface {
	DRWANetworkIdentityCandidates(key []byte) (map[string][]byte, error)
}

func loadAndValidateDRWANetworkIdentity(
	storageService dataRetriever.StorageService,
	key []byte,
	chainID string,
	canonicalEpoch uint32,
	marshalizer marshal.Marshalizer,
	hasher hashing.Hasher,
) ([]byte, drwaNetworkIdentity, [32]byte, [32]byte, bool, error) {
	candidates, err := loadDRWANetworkIdentityCandidates(storageService, key)
	if err != nil {
		return nil, drwaNetworkIdentity{}, [32]byte{}, [32]byte{}, false, err
	}
	if len(candidates) == 0 {
		return nil, drwaNetworkIdentity{}, [32]byte{}, [32]byte{}, false, nil
	}
	...
	var selectedEnvelope []byte
	var selectedIdentity drwaNetworkIdentity
	var selectedHash [32]byte
	var selectedDomain [32]byte
	for _, name := range names {
		envelope := candidates[name]
		identity, decodeErr := decodeDRWANetworkIdentity(envelope, []byte(chainID))
		if decodeErr != nil {
			return nil, drwaNetworkIdentity{}, [32]byte{}, [32]byte{}, false, fmt.Errorf(
				"%w: invalid %s candidate: %w",
				errInvalidDRWANetworkIdentity,
				name,
				decodeErr,
			)
		...
		if identity.epoch != canonicalEpoch {
			return nil, drwaNetworkIdentity{}, [32]byte{}, [32]byte{}, false, fmt.Errorf(
				"%w: %s candidate stored epoch %d, expected %d",
				errInvalidDRWANetworkIdentity,
				name,
				identity.epoch,
				canonicalEpoch,
		...
		canonicalHash, networkDomain, deriveErr := deriveDRWANetworkDomainFromStoredHeader(
			chainID,
			identity,
			marshalizer,
			hasher,
		...
		if selectedEnvelope == nil {
			selectedEnvelope = append([]byte(nil), envelope...)
			selectedIdentity = identity
			selectedHash = canonicalHash
			selectedDomain = networkDomain
			continue
		}
		if !bytes.Equal(selectedEnvelope, envelope) {
			return nil, drwaNetworkIdentity{}, [32]byte{}, [32]byte{}, false, fmt.Errorf(
				"%w: conflicting retained identity candidates %q and %q",
				errInvalidDRWANetworkIdentity,
				names[0],
				name,
			)
	...
	return selectedEnvelope, selectedIdentity, selectedHash, selectedDomain, true, nil
}

func loadDRWANetworkIdentityCandidates(
	storageService dataRetriever.StorageService,
	key []byte,
) (map[string][]byte, error) {
	storer, storerErr := storageService.GetStorer(dataRetriever.DRWANetworkIdentityUnit)
	if storerErr == nil {
		provider, ok := storer.(drwaNetworkIdentityCandidateProvider)
		if ok {
			candidates, err := provider.DRWANetworkIdentityCandidates(key)
			if err != nil {
				return nil, fmt.Errorf("%w: enumerate retained identity candidates: %w", errInvalidDRWANetworkIdentity, err)
			}
			return candidates, nil
		}
	}

	envelope, err := storageService.Get(dataRetriever.DRWANetworkIdentityUnit, key)
	if errors.Is(err, storage.ErrKeyNotFound) {
		return map[string][]byte{}, nil
	}
	if err != nil {
		return nil, fmt.Errorf("%w: read retained identity: %w", errInvalidDRWANetworkIdentity, err)
	}

	return map[string][]byte{"global": append([]byte(nil), envelope...)}, nil
}
