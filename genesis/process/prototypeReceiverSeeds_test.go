...
func TestValidateAndCanonicalizeDRWAReceiverSeeds(t *testing.T) {
	t.Parallel()

	holderOne := drwaReceiverSeedAddress(1)
	holderTwo := drwaReceiverSeedAddress(2)
	configured := []config.DRWAReceiverSeedConfig{
		drwaReceiverSeedConfig(holderTwo, "TOKEN-abcdef"),
		drwaReceiverSeedConfig(holderOne, "ZETA-abcdef"),
		drwaReceiverSeedConfig(holderOne, "ALPHA-abcdef"),
	}
	configured[0].Admitted = false
	arg := drwaReceiverSeedArgs(configured)
	original := append([]config.DRWAReceiverSeedConfig(nil), configured...)

	canonical, err := validateAndCanonicalizeDRWAReceiverSeeds(arg)
	require.NoError(t, err)
	require.Equal(t, original, arg.DRWAReceiverSeeds)
	...
	for _, seed := range canonical {
		require.Equal(t, drwa.ReceiverGateStorageKey(seed.tokenID), seed.receiverStorageKey)
		record, decodeErr := drwa.DecodeReceiverGateRecord(seed.encodedReceiverRecord)
...
func TestValidateAndCanonicalizeDRWAReceiverSeedsEncodesCanonicalInitialBalance(t *testing.T) {
	t.Parallel()

	configured := drwaReceiverSeedConfig(drwaReceiverSeedAddress(1), "TOKEN-abcdef")
	configured.InitialBalance = "340282366920938463463374607431768211455"
	arg := drwaReceiverSeedArgs([]config.DRWAReceiverSeedConfig{configured})
...
func TestValidateAndCanonicalizeDRWAReceiverSeedsEncodesCanonicalFrozenState(t *testing.T) {
	t.Parallel()
...
func TestValidateAndCanonicalizeDRWAReceiverSeedsRejectsEveryInvalidClass(t *testing.T) {
	t.Parallel()

	valid := drwaReceiverSeedConfig(drwaReceiverSeedAddress(1), "TOKEN-abcdef")
	...
	tests := map[string]func(arg *ArgsGenesisBlockCreator){
		"holder decode": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].HolderAddress = "not-hex"
		},
		"holder length": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].HolderAddress = hex.EncodeToString(make([]byte, 31))
		},
		"zero holder": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].HolderAddress = hex.EncodeToString(make([]byte, 32))
		},
		"system holder": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].HolderAddress = hex.EncodeToString(vmcommon.SystemAccountAddress)
		},
		"smart contract holder": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].HolderAddress = hex.EncodeToString(smartContractAddress)
		},
		"invalid token": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].TokenIdentifier = "invalid"
		},
		"zero CEB": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].CEBEpoch = 0
		},
		"mismatched CEB": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].CEBEpoch++
		},
		"zero valid-through round": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].ValidThroughRound = 0
		},
		...
		"duplicate holder and token": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds = append(arg.DRWAReceiverSeeds, arg.DRWAReceiverSeeds[0])
		},
		"zero initial balance": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].InitialBalance = "0"
		},
		"leading-zero initial balance": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].InitialBalance = "01"
		},
		"signed initial balance": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].InitialBalance = "+1"
		},
		"non-decimal initial balance": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].InitialBalance = "1a"
		},
		"over-32-byte initial balance": func(arg *ArgsGenesisBlockCreator) {
			arg.DRWAReceiverSeeds[0].InitialBalance = new(big.Int).Lsh(big.NewInt(1), 256).String()
		},
	}
	...
	for name, mutate := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			arg := drwaReceiverSeedArgs([]config.DRWAReceiverSeedConfig{valid})
			mutate(&arg)
			_, err := validateAndCanonicalizeDRWAReceiverSeeds(arg)
			require.ErrorIs(t, err, ErrInvalidDRWAReceiverSeeds)
		})
	}
}

func TestApplyDRWARegulatedTokenMarkersSortsDeduplicatesAndPreservesFailures(t *testing.T) {
	t.Parallel()

	configured := []config.DRWAReceiverSeedConfig{
		drwaReceiverSeedConfig(drwaReceiverSeedAddress(0), "ZETA-abcdef"),
		drwaReceiverSeedConfig(drwaReceiverSeedAddress(1), "ALPHA-abcdef"),
		drwaReceiverSeedConfig(drwaReceiverSeedAddress(2), "ZETA-abcdef"),
	}
	arg := drwaReceiverSeedArgs(configured)
	canonical, err := validateAndCanonicalizeDRWAReceiverSeeds(arg)
	require.NoError(t, err)
	...
	count, err := applyDRWARegulatedTokenMarkers(arg, canonical)
	require.NoError(t, err)
	require.Equal(t, 2, count)
	require.Equal(t, []string{
		string(mustDRWARegulatedTokenKey(t, "ALPHA-abcdef")),
		string(mustDRWARegulatedTokenKey(t, "ZETA-abcdef")),
	}, observedKeys)
...
func TestApplyDRWAReceiverSeedsFiltersEachShardAndPersistsExactRecord(t *testing.T) {
	t.Parallel()

	configured := []config.DRWAReceiverSeedConfig{
		drwaReceiverSeedConfig(drwaReceiverSeedAddress(0), "ZERO-abcdef"),
		drwaReceiverSeedConfig(drwaReceiverSeedAddress(1), "ONE-abcdef"),
		drwaReceiverSeedConfig(drwaReceiverSeedAddress(2), "TWO-abcdef"),
	}
	...
	for shardID := uint32(0); shardID < 3; shardID++ {
		...
		t.Run(hex.EncodeToString([]byte{byte(shardID)}), func(t *testing.T) {
			...
			count, err := applyDRWAReceiverSeeds(arg, canonical)
			require.NoError(t, err)
			require.Equal(t, 1, count)
			require.Equal(t, byte(shardID), savedAddress[len(savedAddress)-1])
			require.Equal(t, drwa.ReceiverGateStorageKey(canonical[shardID].tokenID), storedKey)
			record, err := drwa.DecodeReceiverGateRecord(storedValue)
			require.NoError(t, err)
...
func TestApplyDRWAReceiverSeedsPreservesEveryStorageFailure(t *testing.T) {
	t.Parallel()

	injected := errors.New("injected receiver seed failure")
	validArg := drwaReceiverSeedArgs([]config.DRWAReceiverSeedConfig{
		drwaReceiverSeedConfig(drwaReceiverSeedAddress(0), "TOKEN-abcdef"),
	})
	canonical, err := validateAndCanonicalizeDRWAReceiverSeeds(validArg)
	require.NoError(t, err)
...
func TestApplyDRWAReceiverSeedsWritesBalanceBeforeReceiverAndRejectsExisting(t *testing.T) {
	t.Parallel()

	configured := drwaReceiverSeedConfig(drwaReceiverSeedAddress(0), "TOKEN-abcdef")
...
func TestApplyDRWAReceiverSeedsAcceptsNilTrieOnlyForEmptyDataRoot(t *testing.T) {
	t.Parallel()
	...
	tests := map[string]struct {
		...
		"dirty exact-key collision remains rejected": {
			...
			expectedError: ErrInvalidDRWAReceiverSeeds,
		},
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			count, applyErr := applyDRWAReceiverSeeds(localArg, canonical)
...
func TestApplyDRWAReceiverSeedsPreservesBalanceAndReceiverWriteFailures(t *testing.T) {
	t.Parallel()
...
func TestCreateShardGenesisBlockRejectsReceiverSeedsOnHardForkBeforeDependencies(t *testing.T) {
	...
	arg := ArgsGenesisBlockCreator{
		...
		DRWAReceiverSeeds: []config.DRWAReceiverSeedConfig{{HolderAddress: "not-evaluated"}},
	}
...
func drwaReceiverSeedArgs(seeds []config.DRWAReceiverSeedConfig) ArgsGenesisBlockCreator {
	...
	return ArgsGenesisBlockCreator{
		...
		DRWAReceiverSeeds: append(
			[]config.DRWAReceiverSeedConfig(nil), seeds...,
		),
	}
}

func drwaReceiverSeedConfig(holderAddress string, tokenIdentifier string) config.DRWAReceiverSeedConfig {
	return config.DRWAReceiverSeedConfig{
...
func drwaReceiverSeedAddress(lastByte byte) string {
...
func mustDRWARegulatedTokenKey(t *testing.T, token string) []byte {
	t.Helper()
	key, err := drwa.DRWARegulatedTokenKey([]byte(token))
	require.NoError(t, err)
	return key
}
