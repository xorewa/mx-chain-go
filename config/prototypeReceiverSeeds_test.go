...
func TestDRWAReceiverSeedsStockConfigIsExplicitlyEmpty(t *testing.T) {
	t.Parallel()

	cfg := &Config{}
	require.NoError(t, core.LoadTomlFile(cfg, "../cmd/node/config/config.toml"))
	require.Empty(t, cfg.BuiltInFunctions.DRWAReceiverSeeds)
}

func TestDRWAReceiverSeedsTOMLDecoding(t *testing.T) {
	t.Parallel()

	tests := map[string]struct {
		tomlInput string
		expected  []DRWAReceiverSeedConfig
	}{
		"empty": {
...
`,
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, test.expected, cfg.BuiltInFunctions.DRWAReceiverSeeds)
		})
	}
}
