...
type gasScheduleNotifier struct {
	...
	versionedSchedules []common.DRWAGasScheduleVersion
...
func NewGasScheduleNotifier(args ArgsNewGasScheduleNotifier) (*gasScheduleNotifier, error) {
	...
	g := &gasScheduleNotifier{
		...
		versionedSchedules: make([]common.DRWAGasScheduleVersion, 0, len(gasScheduleByEpochs)),
	}
	...
	for _, gasScheduleConf := range g.gasScheduleConfig.GasScheduleByEpochs {
		...
		g.versionedSchedules = append(g.versionedSchedules, common.DRWAGasScheduleVersion{
...
// REPLACED_BY_PART_B
// DRWAVersionedGasSchedules returns deep copies of every exact schedule map loaded at startup.
func (g *gasScheduleNotifier) DRWAVersionedGasSchedules() []common.DRWAGasScheduleVersion {
	g.mutNotifier.RLock()
	defer g.mutNotifier.RUnlock()

	result := make([]common.DRWAGasScheduleVersion, len(g.versionedSchedules))
	for index, version := range g.versionedSchedules {
		result[index] = common.DRWAGasScheduleVersion{
