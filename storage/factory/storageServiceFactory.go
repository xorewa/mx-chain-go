...
// REPLACED_BY_PART_B
var errInvalidDRWANetworkIdentityStorage = errors.New("invalid non-normative DRWA prototype network identity storage")
...
func checkArgs(args StorageServiceFactoryArgs) error {
	...
	if args.Config.DRWANetworkIdentityStorage.DB.Type != string(storageunit.LvlDBSerial) {
		return fmt.Errorf("%w: database type must be %s", errInvalidDRWANetworkIdentityStorage, storageunit.LvlDBSerial)
	}
	if args.Config.DRWANetworkIdentityStorage.DB.MaxBatchSize != 1 {
		return fmt.Errorf("%w: maximum batch size must be 1", errInvalidDRWANetworkIdentityStorage)
	}
	if args.Config.DRWANetworkIdentityStorage.DB.BatchDelaySeconds < 1 {
		return fmt.Errorf("%w: batch delay must be positive", errInvalidDRWANetworkIdentityStorage)
	}
	if args.Config.DRWANetworkIdentityStorage.DB.FilePath == "" {
		return fmt.Errorf("%w: database path must be non-empty", errInvalidDRWANetworkIdentityStorage)
	}
...
func (psf *StorageServiceFactory) createAndAddBaseStorageUnits(
	...
	if psf.storageType == ProcessStorageService {
		drwaNetworkIdentityUnit, err := newDRWANetworkIdentityStorer(
			psf.pathManager,
			psf.generalConfig.DRWANetworkIdentityStorage,
		)
		if err != nil {
			return fmt.Errorf("%w for PrototypeNetworkIdentityStorage", err)
		}
		store.AddStorer(dataRetriever.DRWANetworkIdentityUnit, drwaNetworkIdentityUnit)
	}
